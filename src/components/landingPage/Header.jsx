		...
		<header className="sticky top-0 z-50 border-b border-border bg-background/80 backdrop-blur-md transition-all duration-300">
			<nav className="mx-auto flex max-w-7xl items-center justify-between px-6 py-4">
				...
				<div className="flex items-center gap-2">
					...
					<h1 className="font-semibold text-foreground tracking-tight">Cabinet Dr. Amrani</h1>
				</div>
				...
				<div className="hidden md:flex items-center gap-1 bg-secondary/50 p-1 rounded-full border border-border/50 backdrop-blur-sm">
					{navLinks.map((link) => (
						<a
							...
							className={`relative px-4 py-2 text-sm font-medium rounded-full transition-all duration-300 ease-out z-10 ${activeSection === link.id
									? "text-primary-foreground"
									: "text-muted-foreground hover:text-foreground"
								}`}
						>
							{activeSection === link.id && (
								<span className="absolute inset-0 bg-primary rounded-full shadow-md -z-10 animate-scale-in duration-200" style={{ transformOrigin: 'center' }} />
							)}
							{link.name}
						</a>
					))}
				...
				{/* CTA */}
				<Link to="/dashboard">
					<Button className="shadow-none hover:shadow-md transition-shadow">Accès Cabinet</Button>
				</Link>
			</nav>
		</header>
	);
